...
export default function ServiceCard({ service, index }: ServiceCardProps) {
  ...
  // Determine the href based on user authentication
  const getButtonHref = () => {
    if (user) {
      return '/client/projects/create'
    } else {
      return '/auth/login?action=start_project'
    }
  }
  ...
  return (
    ...
    >
      {/* Main Card */}
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden hover:shadow-xl transition-all duration-300 hover:border-orange-300 h-full flex flex-col">
        {/* Card Header */}
        ...
        <div className="p-6 pt-0 mt-auto">
          <Link 
            href={getButtonHref()}
            className={`w-full flex items-center justify-between p-4 rounded-lg bg-gradient-to-r ${gradientClass} text-white hover:shadow-lg transition-all duration-300 ${isHovered ? 'shadow-md' : ''}`}
          >
            <div className="flex items-center gap-2">
              <span className="font-semibold">
                {user ? 'Start Project Now' : 'Get Started'}
              </span>
            </div>
            <div className={`w-8 h-8 rounded-full bg-white/20 flex items-center justify-center transform ${isHovered ? 'translate-x-1' : ''} transition-transform`}>
          ...
          {!user && !loading && (
            <p className="text-xs text-gray-500 text-center mt-2">
              <Link href="/auth/login#signup" className="text-orange-600 hover:text-orange-700 font-medium">
                Create an account
